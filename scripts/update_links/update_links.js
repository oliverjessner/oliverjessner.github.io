...
async function fetchArticleMetadata(articleUrl, searchDocument) {
  ...
  const metaTitle = stripGolemSuffix(getMetaContent(metas, ['title', 'og:title', 'twitter:title']) ?? jsonLd?.headline);
  const title = buildFullTitle(searchDocument.kicker, searchDocument.title) || metaTitle;
  const description = searchDocument.teaserText
    || getMetaContent(metas, ['description', 'og:description', 'twitter:description'])
    || jsonLd?.description
    || '';

  const imageUrl = pickBestCoverImage(searchDocument.coverImages)
...
function buildCategories(searchDocument, metas, jsonLd) {
  const keywords = getMetaContent(metas, ['news_keywords'])
    ?.split(',')
    .map((item) => item.trim())
    .filter(Boolean) ?? [];
  ...
  return uniqueList([...keywords, ...apiTopics, ...jsonLdTopics]
    .map(mapCategory)
    .filter(Boolean))
    .slice(0, 5);
}
...
function stripGolemSuffix(title) {
  return decodeHtmlEntities(title ?? '').replace(/\s+-\s+Golem\.de\s*$/i, '').trim();
}
...
async function downloadAndConvertImages(imageUrl, slug) {
  ...
  try {
    ...
    await convertToWebp(sourcePath, path.join(articleImageDir, 'header_thumbnail.webp'), targetWidth(sourceWidth, 500));
  } finally {
